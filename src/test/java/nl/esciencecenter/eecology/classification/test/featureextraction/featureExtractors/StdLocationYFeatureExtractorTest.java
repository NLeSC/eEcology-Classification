...
public class StdLocationYFeatureExtractorTest extends FeatureExtractorTest {
    @Test
    ...
    public void extractFeatures_1SequenceVector_correctOutput() {
        ...
        gpsSpeed = new DoubleMatrix(x.rows, 1);
        FormattedSegments input = new FormattedSegments(x, y, z, gpsSpeed);
        DoubleMatrix expected = new DoubleMatrix(new double[][] { { Math.sqrt(8 / 1) } });
    ...
    public void extractFeatures_2SequenceVectors_correctOutput() {
        ...
        gpsSpeed = new DoubleMatrix(x.rows, 1);
        FormattedSegments input = new FormattedSegments(x, y, z, gpsSpeed);
        DoubleMatrix expected = new DoubleMatrix(new double[][] { { Math.sqrt(8 / 1) }, { Math.sqrt(2 / 1) } });
    ...
    public void setUp() throws Exception {
        featureExtractor = new StdLocationYFeatureExtractor(new StdExtractor());
    }
}
