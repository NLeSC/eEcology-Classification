...
public class NoiseZFeatureExtractorTest extends FeatureExtractorTest {
    @Test
    ...
    public void extractFeatures_3pointsNonZeroNoise_correctResult() {
        ...
        x = new DoubleMatrix(new double[][] { { 1, 1, 1 } });
        y = new DoubleMatrix(new double[][] { { 1, 1, 1 } });
        z = new DoubleMatrix(new double[][] { { 1, 3, 1 } });
        gpsSpeed = new DoubleMatrix(x.rows, 1);
        FormattedSegments input = createFormattedSegments(x, y, z, gpsSpeed);
    ...
    public void extractFeatures_4points_correctResult() {
        // Arrange
        DoubleMatrix x, y, z, gpsSpeed;
        x = new DoubleMatrix(new double[][] { { 1, 1, 1, 1 } });
        y = new DoubleMatrix(new double[][] { { 1, 1, 1, 1 } });
        z = new DoubleMatrix(new double[][] { { 7, 3, 5, 9 } });
        gpsSpeed = new DoubleMatrix(x.rows, 1);
        FormattedSegments input = createFormattedSegments(x, y, z, gpsSpeed);
    ...
    public void setUp() throws Exception {
        featureExtractor = new NoiseZFeatureExtractor();
    }
}
