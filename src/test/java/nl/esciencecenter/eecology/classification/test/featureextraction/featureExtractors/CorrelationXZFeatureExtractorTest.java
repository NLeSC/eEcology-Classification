...
public class CorrelationXZFeatureExtractorTest extends FeatureExtractorTest {
    ...
    public void extractFeatures_2SequenceVectors_correctOutput() {
        ...
        FormattedSegments input = new FormattedSegments(test1x, test1y, test1z, gpsSpeed);

        // Act
        DoubleMatrix output = featureExtractor.extractFeatures(input);

        // Assert
        assertEquals(test1xzExpected.get(0, 0), output.get(0, 0), errorMargin);
    }

    @Test
    public void extractFeatures_someInput_noNaNs() {
        DoubleMatrix gpsSpeed = new DoubleMatrix(realisticTestx.rows, 1);
        FormattedSegments input = new FormattedSegments(realisticTestx, realisticTesty, realisticTestz, gpsSpeed);
    ...
    public void setUp() throws Exception {
        featureExtractor = new CorrelationXZFeatureExtractor();
    }
}
